...
export class IamController extends Controller {
    ...
    constructor(authenticationService = inject(IamService)) {
        super();
        this.authenticationService = authenticationService;
    }
    routes() {
        return this.controller
            .post("/sign-up/request", zValidator("json", passwordSignUpDto), async (c) => {
            await this.authenticationService.requestSignUp(c.req.valid("json"));
            return c.json({ message: "welcome" });
        })
            .post("/sign-up/verify", zValidator("json", passwordSignUpVerifyDto), async (c) => {
            await this.authenticationService.verifySignUp(c.req.valid("json"));
            return c.json({ message: "welcome" });
        })
            .post("/sign-in", zValidator("json", passwordSignUpDto), async (c) => {
            const session = await this.authenticationService.signIn(c.req.valid("json"));
